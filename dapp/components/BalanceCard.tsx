...
export default function BalanceCard() {
  const { balance, balanceVerified, isLoadingBalance, refreshBalance } = useWallet();
  ...
  const handleLoadBalance = () => {
    ...
    // x402 hathor-direct flow only uses HTR for now.
  ...
  if (!isConnected) {
    return (
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700 mb-6">
        <div className="text-lg font-bold text-white mb-4">YOUR BALANCE</div>
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          ...
          {balanceVerified && balance > 0n ? (
            <span className="text-white font-medium">{formatBalanceWithCommas(balance)} HTR</span>
          ) : isLoadingBalance ? (
